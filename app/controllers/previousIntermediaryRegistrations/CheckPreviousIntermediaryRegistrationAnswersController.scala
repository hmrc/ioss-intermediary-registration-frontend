...
import pages.previousIntermediaryRegistrations.CheckPreviousIntermediaryRegistrationAnswersPage
...
class CheckPreviousIntermediaryRegistrationAnswersController @Inject()(
                                                                        override val messagesApi: MessagesApi,
                                                                        cc: AuthenticatedControllerComponents,
                                                                        formProvider: CheckPreviousIntermediaryRegistrationAnswersFormProvider,
                                                                        view: CheckPreviousIntermediaryRegistrationAnswersView
  ...
  def onPageLoad(waypoints: Waypoints, countryIndex: Index): Action[AnyContent] = cc.authAndGetData().async {
    implicit request =>
      ...
      getPreviousCountry(waypoints, countryIndex) { country =>
        ...
        val preparedForm = request.userAnswers.get(CheckPreviousIntermediaryRegistrationAnswersPage(countryIndex)) match {
          case None => form
    ...
    implicit request =>
      ...
      getPreviousCountry(waypoints, countryIndex) { country =>
        ...
        form.bindFromRequest().fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(CheckPreviousIntermediaryRegistrationAnswersPage(countryIndex), value))
              _ <- cc.sessionRepository.set(updatedAnswers)
            } yield Redirect(CheckPreviousIntermediaryRegistrationAnswersPage(countryIndex).navigate(waypoints, request.userAnswers, updatedAnswers).route)
        )
      }
  }
}
