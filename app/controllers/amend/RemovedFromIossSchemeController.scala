...
class RemovedFromIossSchemeController @Inject()(
                                                 override val messagesApi: MessagesApi,
                                                 cc: AuthenticatedControllerComponents,
                                                 frontendAppConfig: FrontendAppConfig,
                                                 registrationService: RegistrationService,
                                                 clock: Clock,
                                                 view: RemovedFromIossSchemeView
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = cc.authAndRequireIntermediary(waypoints, inAmend = true).async {
    implicit request =>
      ...
      val reinstateDeadlineDateFormatted: String = LocalDate.now(clock).plusMonths(1).withDayOfMonth(1).format(dateFormatter)

      registrationService.amendRegistration(
        request.userAnswers,
        request.registrationWrapper.etmpDisplayRegistration,
        request.vrn,
        ...
        case Right(_) =>
          Ok(view(yourAccountUrl, reinstateDeadlineDateFormatted))
