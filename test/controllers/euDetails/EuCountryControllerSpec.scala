...
class EuCountryControllerSpec extends SpecBase with MockitoSugar {
  ...
  private val form = formProvider(euCountries)

  private lazy val euCountryRoute = routes.EuCountryController.onPageLoad(waypoints).url
  ...
  "EuCountry Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = updatedAnswers.set(EuCountryPage, country).success.value
      ...
      running(application) {
        ...
        status(result) `mustBe` OK
        contentAsString(result) `mustBe` view(form.fill(country), waypoints)(request, messages(application)).toString
      }
    }
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value `mustBe` EuCountryPage.navigate(waypoints, emptyUserAnswersWithVatInfo, expectedAnswers).url
