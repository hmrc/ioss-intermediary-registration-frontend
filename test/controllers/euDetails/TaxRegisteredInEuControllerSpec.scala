...
class TaxRegisteredInEuControllerSpec extends SpecBase with MockitoSugar {
  ...
  "TaxRegisteredInEu Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswersWithVatInfo.set(TaxRegisteredInEuPage, true).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value `mustBe` TaxRegisteredInEuPage.navigate(waypoints, emptyUserAnswersWithVatInfo, expectedAnswers).url
