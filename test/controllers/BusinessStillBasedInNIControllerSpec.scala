...
class BusinessStillBasedInNIControllerSpec extends SpecBase with MockitoSugar {
  ...
  "BusinessStillBasedInNI Controller" - {
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, businessStillBasedInNIRoute)
            .withFormUrlEncodedBody(("value", "true"))

        val expectedAnswers = emptyUserAnswers.set(BusinessStillBasedInNIPage, true).success.value
        val result = route(application, request).value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual BusinessStillBasedInNIPage.navigate(waypoints, emptyUserAnswers, expectedAnswers).url
      }
    }
