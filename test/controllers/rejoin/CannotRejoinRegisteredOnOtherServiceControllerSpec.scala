...
class CannotRejoinRegisteredOnOtherServiceControllerSpec extends SpecBase {
  ...
  "CannotRejoinRegisteredOnOtherService Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.CannotRejoinRegisteredOnOtherServiceController.onPageLoad(countryCode).url)

        val result = route(application, request).value

        val view = application.injector.instanceOf[CannotRejoinRegisteredOnOtherServiceView]
    ...
    "must throw an Exception if country doesn't exist" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.CannotRejoinRegisteredOnOtherServiceController.onPageLoad(invalidCountryCode).url)
