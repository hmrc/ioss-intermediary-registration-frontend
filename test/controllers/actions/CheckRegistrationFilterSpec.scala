  ...
  ".filter" - {
    ...
    "must redirect to Already Registered Controller when an existing Intermediary enrolment is found" in {
      ...
      running(app) {
        ...
        val request = AuthenticatedIdentifierRequest(FakeRequest(), testCredentials, vrn, Enrolments(Set(enrolment)), None, 1, None, None, None, None)
    ...
    "must return None when an existing Intermediary enrolment is not found" in {
      ...
      running(app) {
        val config = app.injector.instanceOf[FrontendAppConfig]
        val request = AuthenticatedIdentifierRequest(FakeRequest(), testCredentials, vrn, Enrolments(Set.empty), None, 1, None, None, None, None)
        val controller = new Harness(false, config)
